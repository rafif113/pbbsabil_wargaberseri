    						...
    						<tbody>
    							...
    							<?php foreach ($musrembang as $key) : ?>
    								<tr>
    									...
    									<td><?= $key['status'] ?></td>
    									<td>
    										<a href="<?= base_url("admin/admin/updateMusrembang/$key[id]"); ?>" class="badge badge-success updateMusrembangModalButton" data-toggle="modal" data-target="#newMusrembangModal" data-id="<?= $key['id'] ?>">Ubah</a>
    										<a href="<?= base_url("admin/admin/deleteMusrembang/$key[id]"); ?>" class="badge badge-danger" onclick="return confirm('Are you sure?');">Hapus</a>
    									</td>
    								</tr>
    								<?php $no++; ?>
    							<?php endforeach ?>
